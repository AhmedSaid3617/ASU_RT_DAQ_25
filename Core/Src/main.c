...
/*   IMU_tAxisMap loc_structMapping = {
  ...
  /* ========================================== TRAVEL INIT ========================================== */
  ADC_DEV_start(&hdma_adc1, &hadc1, ADC_travel_readings);
  /* ======================================== TRAVEL INIT END ======================================== */

  xTaskCreate(green_task, "Green Task", 128, NULL, 1, &task_handles[0]);
  xTaskCreate(IMU_task, "External Pin Task", 128, NULL, 1, &task_handles[1]);
  xTaskCreate(COMM_CAN_send_task, "CAN Task", 128, NULL, 1, &task_handles[2]);
  xTaskCreate(TRAVEL_task, "Travel Task", 128, NULL, 1, &task_handles[3]);
...
void TRAVEL_task(){
  ...
  while (1)
  {
    ...
    vTaskDelay(CONFIG_TRAVEL_TASK_WAIT);
  }
